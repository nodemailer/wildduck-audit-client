...
class Audits {
    ...
    async getSignedLog(audit, entry) {
        ...
        const cleartext = `File name: ${logData.metadata.fileName}
Account: ${auditData.meta.name} <${auditData.meta.address}> (${auditData.meta.username})
Audit ID: ${auditData._id}
Downloaded file size:  ${logData.metadata.bytes} bytes
Included emails: ${logData.metadata.messageCount}
Hash (${logData.metadata.algo}): ${logData.metadata.hash}
Downloader: ${logData.metadata.owner.name} (${logData.metadata.owner.username})
Created: ${logData.metadata.curtime.toISOString()}`;
